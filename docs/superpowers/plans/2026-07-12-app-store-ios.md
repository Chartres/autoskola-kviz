...
**Constraint (flywheel PR 15 — fleet auth hardening):** autoskola's magic-link emails are sent by a shared Supabase Send-Email hook that branches on `redirect_to`, and `fleet-auth-check.mjs` asserts each app's redirect allowlist weekly. A native shell's `window.location.href` is `capacitor://localhost/…` — not in the allowlist, not handled by the hook. Therefore: **on native, hide the email-OTP form entirely** (AuthPanel renders only the Apple/Google button per platform); the email flow stays web-only and unchanged. Deep-link magic links are a v2 item that must go through flywheel's `docs/standards/auth-identity.md` + allowlist update — do not add them here.
